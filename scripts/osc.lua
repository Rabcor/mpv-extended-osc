...
local user_opts = {
    ...
    loop_mbtn_left_command = "script-message-to osc osc-loop",
    loop_mbtn_mid_command = "",
    loop_mbtn_right_command = "",


    shuffle_mbtn_left_command = "script-message-to osc osc-shuffle",
    shuffle_mbtn_mid_command = "",
    shuffle_mbtn_right_command = "",
    -- luacheck: pop
}
...
local function bar_layout(direction, slim)
    ...
    -- Right timecode
    geo = { x = geo.x - tcW + padX, y = geo.y, an = geo.an,
            w = tcW, h = geo.h }
    lo = add_layout("tc_right")
    lo.geometry = geo
    lo.style = osc_styles.timecodesBar
...
local function osc_init()
    ...
    ne.content = function ()
        ...
        return mp.get_property("loop") ~= "no" and icons.loop or icons.loop
    end
    bind_mouse_buttons("loop")
    ...
    ne.enabled = have_pl
    ne.content = icons.shuffle
    bind_mouse_buttons("shuffle")
...
mp.register_script_message("osc-loop", function ()
    mp.commandv("cycle-values","loop-file","yes","no")
    if	mp.get_property_native("loop-file") ~= false then
    	 mp.osd_message("Looping File On")
    else
    	mp.osd_message("Looping File Off")
	end
end)

mp.register_script_message("osc-shuffle", function ()
