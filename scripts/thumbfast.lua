...
--
-- Stripped of cross platform code (This variant is linux only)
--
-- Replaced max_width/max_height with a scale value that resizes video relative to window instead.
--
--[[
...
local options = {
    ...
    thumbnail = "/tmp/thumbfast.out",

    -- Scale of the preview relative to the main window size; not 100% accurate but good enough.
    scale = 0.125,
...
local effective_w, effective_h = 100, 100
local real_w, real_h
...
local function calc_dimensions()
    local width = properties["video-out-params"] and properties["video-out-params"]["dw"]
    local height = properties["video-out-params"] and properties["video-out-params"]["dh"]
    local window = mp.get_property("current-window-scale")
    if not width or not height or not window then return end

    local dpi_scale = properties["display-hidpi-scale"] or 1
	
	effective_h = math.floor(height * options.scale * window * dpi_scale)
    effective_w = math.floor(width * options.scale * window * dpi_scale)
...
local function info(w, h)
    ...
    local json, err = mp.utils.format_json({width=w, height=h, scale_factor=1, disabled=disabled, available=true, socket=options.socket, thumbnail=options.thumbnail, overlay_id=options.overlay_id})
...
local function draw(w, h, script)
    ...
    if x ~= nil then
        mp.command_native_async({"overlay-add", options.overlay_id, x, y, options.thumbnail..".bgra", 0, "bgra", w, h, (4*w), w, h}, function() end)
    elseif script then
        local json, err = mp.utils.format_json({width=w, height=h, scale_factor=1, x=x, y=y, socket=options.socket, thumbnail=options.thumbnail, overlay_id=options.overlay_id})
